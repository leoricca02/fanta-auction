...
describe('§6.1 — export nativo reimportabile', () => {
  ...
  it('conserva tutte le 537 righe, fuori lista compresi', () => {
    // Il parser ne scarta 21, ma il file per la lega deve restare integro.
    const result = buildNativeXlsx(readListoneBytes(), assignmentsFromState(state, config));
    expect(readBack(result.bytes)).toHaveLength(537);
  });
  ...
  it('il file esportato e ancora leggibile dal nostro parser', () => {
    ...
    expect(reparsed.players).toHaveLength(516);
    expect(reparsed.excludedCount).toBe(21);
  });
